...
| 공용 | `statistics.py`/`charts.py` | `StatisticsPanel`/`StatisticsPage` 등 | 통계 대시보드 본문(수집 상태 진단 배너 + 단일 스크롤 화면 + 접이식 상세 정보, 독립 패널 위젯) + 이를 감싼 페이지, 커스텀 차트(`RankedBarChart`/`GroupedBarChart`) |
