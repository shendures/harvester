...
| multi | `main_window.py` | `MainWindowMulti(MainWindowBase)` | 블루프린트별 페이지 번들 지연 생성·관리 |
...
| 파일 | 클래스 | 기능 |
|---|---|---|
| `common.py` | (다수 함수) | 공용 허브 — task 설정 적용, 블루프린트 검증(`engine`), 추출/DB 설정 UI 빌더, 모달 다이얼로그 골격·버튼 행(`_build_modal_dialog`/`_build_dialog_button_row`) |
