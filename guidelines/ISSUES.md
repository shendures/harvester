...
| ⑳ | `custom_rules/render/{seq_no}.py` 서브폴더 미이관 — render/refine 분리 리팩터링 중 000010(맥도날드)·000013(네이버) 원본이 삭제만 되고 재이관 안 됨 | `custom_rules/render/`(부재), `conf.py`(`CustomModuleStorage`) | ✅ 해결 |
...
| ㉕ | PyInstaller 배포 파이프라인이 저장소에 없어 정제/수집 커스텀 규칙이 exe에 실제로 번들되는지 보장할 방법이 없었음 — `PREPROCESS.md`가 안내하던 배포 경로(레포 루트 평면 구조)도 `CustomModuleStorage`의 실제 조회 경로(`custom_rules/{kind}/{seq_no}.py`)와 불일치 | `guidelines/PREPROCESS.md`, (부재였던) 빌드 스크립트 | ✅ 해결 |
...
| ⑳ | **`custom_rules/render/{seq_no}.py` 서브폴더 미이관** — 수집(render, Selenium 자식 프로세스)과 정제(refine, 메인 GUI 프로세스)의 실행 컨텍스트가 달라 `custom_rules/render/`·`custom_rules/refine/`로 분리하는 리팩터링을 진행하던 중, 기존 `custom_rules/000010.py`(맥도날드 전용 `render()`)·`000013.py`(네이버 전용 `login()`)가 git에서 삭제만 되고 `custom_rules/render/`로 재이관되지 않은 채 중단됨. `custom_rules/refine/000000.py`(샤브올데이 정제)만 정상 이관 완료. 현재 `custom_rules/render/` 폴더 자체가 디스크에 존재하지 않음 | `custom_rules/render/`(부재), `conf.py`(`CustomModuleStorage`) | ✅ **해결** — 삭제 커밋(`53978d0`)의 부모 커밋에서 두 파일의 원본이 git 이력에 그대로 남아있음을 확인, `git show 53978d0^:custom_rules/000010.py`·`000013.py`로 내용을 그대로 복원해 `custom_rules/render/000010.py`·`render/000013.py`로 재이관(파일 상단 경로 주석만 새 위치에 맞게 수정, 로직은 변경 없음) |
...
| ㉕ | **PyInstaller 배포 파이프라인이 저장소에 없어 정제/수집 커스텀 규칙 번들 여부를 보장할 방법이 없음** — (exe 배포 검토 중 발견) `.spec` 파일이 `.gitignore`에 등록돼 있고 빌드 스크립트도 저장소 어디에도 없어, `custom_rules/`·`request_info.json`을 실제 exe에 포함시키는 `--add-data` 구성이 전적으로 수동·비문서화 상태였음. 게다가 `PREPROCESS.md`가 안내하던 배포 절차("`custom_rules/{seq_no}.py`를 레포 루트로 복사")는 render/refine 서브폴더 분리(`53978d0`) 이전의 평면 경로 기준이라, 그대로 따르면 `CustomModuleStorage.resolve_path()`가 실제로 찾는 `custom_rules/{kind}/{seq_no}.py` 위치와 어긋나 시딩이 조용히 실패함(크래시 없음, 커스텀 규칙만 누락) | `guidelines/PREPROCESS.md`, (부재였던) 빌드 스크립트 | ✅ **해결** — 레포 루트에 `build-exe.ps1`을 추가: `request_info.json`의 `seq_no`와 `-SeqNo` 인자가 일치하는지 먼저 검증(다른 고객 파일 오조합 방지)한 뒤, 해당 seq_no의 `custom_rules/refine/`·`render/` 파일만 임시 스테이징 폴더에 모아 `--add-data`로 PyInstaller에 전달 — `custom_rules/` 전체를 통째로 번들에 넣지 않으므로 다른 고객의 규칙 파일이 섞여 들어갈 위험도 없음. `PREPROCESS.md` §3.1·§3.1a·§4·§5의 배포 경로 안내를 `custom_rules/{kind}/{seq_no}.py` 및 `build-exe.ps1` 사용법 기준으로 갱신 |
