...
# layout/single·layout/multi의 각 클래스가 해당 Mixin을 다중상속하여 메서드를
# 주입받습니다 — 단일 수집 레이아웃과 다중 수집 레이아웃이 이 패키지를 공유합니다.
# 서브모듈은 화면 단위로 나뉩니다
...
# QWidget 계열 메서드와 충돌하지 않으며, self.xxx 속성은 페이지 클래스의
# _build()에서 이미 생성되어 있으므로 참조 안전합니다.
#
# 이 파일은 `from trigger import X` 호출부가 서브모듈 경로를 몰라도 되도록
# 서브모듈의 공개 클래스만 재-export하는 facade입니다.
